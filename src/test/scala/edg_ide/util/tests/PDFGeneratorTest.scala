...
class PDFGeneratorTest extends AnyFlatSpec with Matchers {
  ...
  it should "generate a working PDF file" in {
    val blockIr = elem.HierarchyBlock(
      blocks = SeqMap(
        "R1" -> elem.BlockLike(`type` =
          elem.BlockLike.Type.Hierarchy(elem.HierarchyBlock(
            ...
            blocks = SeqMap(
              "inner_source_block" -> elem.BlockLike(`type` =
                elem.BlockLike.Type.Hierarchy(elem.HierarchyBlock(
                  ports = SeqMap(
                    "inner_source_port" -> elem.PortLike(is =
                      elem.PortLike.Is.Port(elem.Port(
                        selfClass = Some(EdgirTestUtils.Ports.PowerSource),
                      ))
                    ),
        ...
        "R2" -> elem.BlockLike(`type` =
          elem.BlockLike.Type.Hierarchy(elem.HierarchyBlock(
            ...
            blocks = SeqMap(
              "inner_sink_block" -> elem.BlockLike(`type` =
                elem.BlockLike.Type.Hierarchy(elem.HierarchyBlock(
                  ports = SeqMap(
                    "inner_sink_port" -> elem.PortLike(is =
                      elem.PortLike.Is.Port(elem.Port(
                        selfClass = Some(EdgirTestUtils.Ports.PowerSource),
                      ))
                    ),
      ...
      links = SeqMap(
        "link" -> elem.LinkLike(`type` =
          elem.LinkLike.Type.Link(elem.Link(
            ...
            ports = SeqMap(
              "R1_port" -> elem.PortLike(is =
                elem.PortLike.Is.Port(elem.Port(
                  selfClass = Some(EdgirTestUtils.Ports.PowerSource)
                ))
              ),
              "R2_port" -> elem.PortLike(is =
                elem.PortLike.Is.Port(elem.Port(
                  selfClass = Some(EdgirTestUtils.Ports.PowerSource)
                ))
              ),
      ...
      constraints = SeqMap(
        "connect_R1" -> expr.ValueExpr(expr =
        ...
        "connect_R2" -> expr.ValueExpr(expr =
          expr.ValueExpr.Expr.Connected(expr.ConnectedExpr(
