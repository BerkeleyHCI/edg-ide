...
object InsertFootprintAction {
  protected def insertFootprint(
    ...
    val newCall = psiElementGenerator
      .createFromText(
        languageLevel,
        classOf[PyStatement],
  ...
  protected def createModifyFootprint(
    ...
    val argExpr = call
      .getArgument(1, "footprint", classOf[PyExpression])
      .exceptNull("call has no footprint argument")

    val psiElementGenerator = PyElementGenerator.getInstance(project)
    val newExpr = psiElementGenerator
      .createStringLiteralFromString(footprint)
  ...
  def createInsertFootprintFlow(
    ...
    val modifyAction = exceptable {
      ...
      val modifyAction = createModifyFootprint(
        containingCall,
        footprint,
    ...
    val insertAction = exceptable {
      ...
      val insertAction = insertFootprint(
        after,
        footprint,
        selfName,
