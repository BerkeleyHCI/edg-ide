...
object InsertPinningAction {
  case class SelectPortItem(portPath: ref.LocalPath, assigned: Boolean) {
    override def toString: String =
      ExprToString(portPath) + Option.when(assigned)(" (assigned)").getOrElse("")
  ...
  def createInsertPinningFlow(
    ...
    val containingPsiFunction = PsiTreeUtil
      .getParentOfType(after, classOf[PyFunction])
    ...
    val selfName = containingPsiFunction.getParameterList.getParameters.toSeq.headOption
      .exceptNone(s"function ${containingPsiFunction.getName} has no self")
      .getName

    val containingCall = PsiTreeUtil
      .getParentOfType(after, classOf[PyCallExpression])
      .exceptNull(s"not in a function call")
    requireExcept(
      containingCall.getCallee.textMatches(s"$selfName.footprint"),
      "not in a self.footprint call"
    )
    val argDict = containingCall
      .getArgument(2, "pinning", classOf[PyExpression])
    ...
    val circuitPortItems = block.ports.asPairs
      .flatMap { case (name, port) =>
        val portPath = ExprBuilder.Ref(name)
        circuitPortsOf(portPath, port, project)
      }
      .toSeq
      .exceptEmpty(s"block contains no CircuitPorts")
      ...
      val newElt = matchingKv match {
        case Some(kv) => // modify existing
          ...
          writeCommandAction(project)
            .withName(s"Update pin $pin in ${blockClass.getName}")
            .compute(() => {
              kv.getValue.replace(newValue)
            })

        case None => // append new
          val newKv = psiElementGenerator
          ...
          writeCommandAction(project)
            .withName(s"Set pin $pin in ${blockClass.getName}")
            .compute(() => {
