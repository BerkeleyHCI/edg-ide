...
object InsertConnectAction {
  ...
  private def pairAttributesAfter(
      after: PsiElement,
      portPairs: Seq[(String, String)],
  ...
  def createInsertConnectFlow(
    ...
    def insertConnectFlow(): Unit = {
      if (requiresName) {
        InsertAction.createClassMemberNameEntryPopup(
          "Connect Name (optional)",
          containingPsiClass,
          project,
          allowEmpty = true
        ) { name => exceptable { insertConnectAction(name) } }
      } else {
        insertConnectAction("")
      }
    }
  ...
  def createAppendConnectFlow(
      within: PsiElement,
      allPortPairs: Seq[(String, String)],
      portPairs: Seq[(String, String)],
      actionName: String,
      project: Project,
      continuation: (String, PsiElement) => Unit
    ...
    requireExcept(
      containingPsiCall.getArguments.forall { arg =>
