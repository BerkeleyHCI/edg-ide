...
/** Removes links (as edges - must run AFTER collapse - prevents weird interactions with bridge removal)
  * that are "high-fanout", based on the link type allowlist and parameterized number of sink connections.
  */
class RemoveHighFanoutEdgeTransform(minConnects: Int, allowedLinkTypes: Set[LibraryPath]) {
  def apply(node: EdgirGraph.EdgirNode): EdgirGraph.EdgirNode = {
    val highFanoutLinks = node.edges.collect { case EdgirEdge(EdgeLinkWrapper(linkPath, linkLike), source, target) =>
